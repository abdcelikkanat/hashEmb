...
def compute_score(g, included_list):
    ...
    '''
    num_of_nodes = float( len(included_list) )
    sub_g = nx.subgraph(g, included_list)
    in_edge_count = float(sub_g.number_of_edges())
    out_edge_count = float( num_of_nodes * (num_of_nodes-1.0) / 2.0  )
...
def assign_label(g, node_labels, node, new_label):
    ...
    if len(node_labels[int(node)]) > 0:
        ...
        for c in node_labels[int(node)]:
            ...
            if len(set(_clusters[c]).difference(included_list)) == 0 and len(set(included_list).difference(_clusters[c])) == 0:
                do_change = False

                for nb in included_list:
                    if len(node_labels[int(nb)]) == 0:
                        raise ValueError("CUSS YAAA!")

    if do_change:

        # Label each node
        for inc_node in included_list:
            node_labels[int(inc_node)].append(len(_clusters))

        _clusters.append(included_list)
